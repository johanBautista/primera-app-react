  ...
  const keyPress = (letter) => {
    ...
    if (letter === "{bksp}") {
      if (palabraActual.contenido.length > 0) {
        palabrasCopy[indicePalabraActual].contenido = palabraActual.contenido.substring(
          0,
          palabraActual.contenido.length - 1
        );
      }
      ...
      <div className="container-fluid teclado">
        <div className="row inner-teclado">
          <Keyboard
            onKeyPress={keyPress}
            layout={{
              default: ["Q W E R T Y U I O P", "A S D F G H J K L Ñ", "{enter} Z X C V B N M {bksp}"],
            }}
            display={{
